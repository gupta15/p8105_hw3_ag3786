...
###Summarizing the dataset
We will examine the ny_noaa dataset. This dataset has `r nrow(noaa)` observations and `r ncol(noaa)` variables. The variables `id`, `tmax`and `tmin` are of class `character`, whereas  `prcp`, `snow` and `snwd` are of class `integer`. 
...
* Most commonly observed values for snowfall are `r noaa %>% pull(snow) %>% median(na.rm = TRUE)`
